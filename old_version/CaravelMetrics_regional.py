...
'''
def compute_betti_numbers(mask, skeleton_graph):
    """Compute Betti numbers using skeleton + volume."""
    
    # --- Betti 0: from skeleton graph (number of connected components) ---
    b0 = nx.number_connected_components(skeleton_graph)

    # --- Euler number from the full volume using scikit-image ---
    chi = euler_number(mask.astype(np.uint8), connectivity=3)

    # --- Betti 2: number of cavities in the volume ---
    struct = generate_binary_structure(3, 1)  # 6-connectivity
    background = ~mask.astype(bool)
    _, num_bg_components = label(background, structure=struct)
    b2 = max(0, num_bg_components - 1)  # Exclude outer background

    # --- Betti 1: via Euler-Poincaré formula ---
    b1 = b0 + b2 - chi
...
def compute_metrics_for_mask(mask, dist_map, affine, header, selected_metrics,save_segment_masks,save_conn_comp_masks):
...
def register_atlas_to_segmentation(
    atlas_path,
    segmentation_path,
    output_registered_atlas_path,
    mask_atlas=None,
...
def process(image_path, image_T1, mask_path, atlas_path, selected_metrics, save_segment_masks, save_conn_comp_masks):
    """
    ...
    # TEMP path to save registered atlas
    ...
    print(reg_atlas_path)

    fslroi = fsl.ExtractROI()
    fslroi.inputs.in_file = image_T1
    fslroi.inputs.t_min = 0  # No cropping along time dimension
    fslroi.inputs.t_size = -1  # Keep all time points if 4D
    fslroi.inputs.x_min = 0
    fslroi.inputs.x_size = -1 # Keep full extent in X
    fslroi.inputs.y_min = 50
    fslroi.inputs.y_size = -1  # Keep full extent in Y
    fslroi.inputs.z_min = 0  # Start cropping from this slice index in Z
    fslroi.inputs.z_size = -1  # Number of slices to keep in Z from z_min


    result = fslroi.run()
    # Extracted file path from outputs
    ...
    print('Extract Brain mask')
    bet = fsl.BET()
    bet.inputs.in_file = cropped_path # Input image path
    bet.inputs.mask = True  # Generate brain mask output
    bet.inputs.frac = 0.30 # smaller values becomes more conservative - keeps more tissue
    ...
    print('Reorient to standard')
    reorient = fsl.Reorient2Std()
    reorient.inputs.in_file = image_path_no_brain
    reorient.inputs.out_file = image_path_no_brain  # overwriting same file
    reorient.run()

    # Register atlas to segmentation
    ...
    flirt.inputs.interp = 'trilinear'  # or 'nearestneighbour' for label images
    flirt.inputs.apply_xfm = True  # Apply transform (identity here)
    ...
    flirt.inputs.interp = 'nearestneighbour'  # or 'nearestneighbour' for label images
    flirt.inputs.apply_xfm = True  # Apply transform (identity here)
    flirt.inputs.uses_qform = True
    # Run the command
    result = flirt.run()
    ...
    # Create a 3D volume with labeled regions matching the atlas
    labeled_vessel_mask = np.zeros_like(atlas, dtype=np.int16)

    # Iterate through each atlas region and assign vessel voxels that label
    # for label in np.unique(atlas):
    #     if label == 0:
    #         continue  # skip background
        
    #     # Find voxels that are both in this atlas region AND in the segmentation
    #     region_mask = (atlas == label) & (arr_seg == 1)
        
    #     # Assign the atlas label to these voxels in the output mask
    #     labeled_vessel_mask[region_mask] = label

    # # Save the labeled vessel mask
    # labeled_mask_path = os.path.join(os.path.dirname(mask_path), f"{image_name}_labeled_vessels.nii.gz")
    # labeled_mask_nib = nib.Nifti1Image(labeled_vessel_mask, affine_seg, header_seg)
    # nib.save(labeled_mask_nib, labeled_mask_path)
    # print(f"Labeled vessel mask saved to: {labeled_mask_path}")
    ...
    for label in np.unique(atlas):
        ...
        if not np.any(region_mask):
            print(f'No segmentation in region {label}')
            continue

        region_result = compute_metrics_for_mask(
            region_mask, dist_map, affine_seg, header_seg, selected_metrics, save_segment_masks, save_conn_comp_masks
        )
...
if __name__ == '__main__':
    p = argparse.ArgumentParser(description="Vessel metrics computation (with atlas-based regional analysis)")
    p.add_argument('image_path', help='Path to vessel image (.nii/.nii.gz or .npy/.npz)')
    p.add_argument('image_T1',  help='Path to T1 image (.nii/.nii.gz or .npy/.npz)')
    p.add_argument('mask_path', help='Path to vessel mask (.nii/.nii.gz or .npy/.npz)')
    p.add_argument('atlas_path', help='Path to atlas file (.nii/.nii.gz) for regional parcellation')
    p.add_argument('--metrics', nargs='+', default=None, help='Metrics to compute')
    p.add_argument('--output_folder', default='./ATLAS_VESSEL_METRICS', help='Save directory')
    p.add_argument('--no_segment_masks', action='store_true', help='Disable segment mask construction and saving')
    p.add_argument('--no_conn_comp_masks', action='store_true', help='Disable connected component reconstruction saving')
    ...
    results = process(args.image_path, args.image_T1,
        args.mask_path,
        args.atlas_path,
        selected_metrics,
